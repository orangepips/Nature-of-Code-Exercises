...
class Surface {
  ...
  Surface() {
    ...
    float radians = random(0, TWO_PI);
    float increment = QUARTER_PI/60; // the larger the denominator the fewer the waves
    for (int x = 0; x <= width; x++) {
      int y = int(map(sin(radians), -1, 1, height/2, height));
